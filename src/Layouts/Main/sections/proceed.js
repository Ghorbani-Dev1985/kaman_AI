...
function Proceed({chartresponse}){
    ...
    const [selectedOption1_value, setSelectedOptionValue1] = useState(options_keys[selectedOption1]);

    const [selectedOption2, setSelectedOption2] = useState('فروش خالص');
    const [selectedOption2_value, setSelectedOptionValue2] = useState(options_keys[selectedOption2]);
    ...
    return (
        <>
            <div className="w-full flex justify-center items-center p-3">
              <div className='w-full bg-white dark:bg-navy-700 dark:text-white flex flex-col rounded-md overflow-x-auto'>
              <div className="bg-blue-100 dark:bg-navy-600 dark:text-white mb-4 p-3 flex justify-between items-center">
                    <div className="text">
                        عملکرد مشتریان
                    </div>
                ...
                <div className="chart p-2 ">
                    <div className="w-full md:w-2/5 flex items-center gap-3 mb-4">
                        <div className='w-full flex flex-1'>
                            <Select onChange={ChangeValue1}  options={options} placeholder={selectedOption1}  className="w-full text-navy-500" theme='neutral50' />
                        </div>
                        <div className='w-full flex flex-1'>
                            <Select onChange={ChangeValue2}  options={options} placeholder={selectedOption2} className="w-full text-navy-500" theme='neutral50' />
                        </div>
                    </div>
                    <div className="w-full h-auto overflow-x-scroll">
                        {
                            chartresponse[selectedOption1_value ] !== undefined && (
                              <DrawChart chartresponse={chartresponse} selected1={selectedOption1_value}
                              selected2={selectedOption2_value} select_values = {[selectedOption1, selectedOption2]}/>
                            )
                        }
